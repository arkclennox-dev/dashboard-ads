  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      const body = {
        ...
        status,
        products: selected.map((product_id, i) => ({ product_id, sort_order: i })),
      };
  ...
  return (
    <form onSubmit={onSubmit} className="grid max-w-3xl grid-cols-1 gap-4">
      <label className="block">
        ...
        <input className={inputCls} value={title} onChange={(e) => onTitleChange(e.target.value)} required />
      </label>
      <label className="block">
      ...
      <label className="block">
        ...
        <textarea className={inputCls} rows={2} value={intro} onChange={(e) => setIntro(e.target.value)} />
      </label>
      ...
      <fieldset className="rounded-lg border border-border bg-surface-2 p-4">
        ...
        {products.length === 0 ? (
          <p className="text-sm text-muted">No products yet — create one in Redirect Builder first.</p>
        ) : (
      ...
      <div className="flex items-center gap-3">
        ...
        <a href="/landing-pages" className="text-sm text-muted hover:text-ink-2">Cancel</a>
      </div>
    </form>
  );
}
