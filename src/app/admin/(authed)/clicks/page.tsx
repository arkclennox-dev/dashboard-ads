            ...
            <tbody>
              {items.map((c) => (
                ...
                >
                  ...
                  <td>
                    <Badge value={c.is_duplicate} />
                  </td>
                  <td>
                    <Badge value={c.is_bot} variant="warn" />
                  </td>
                </tr>
              ))}
            </tbody>
...
function Badge({ value, variant = "muted" }: { value: boolean; variant?: "muted" | "warn" }) {
  if (!value) {
    return <span className="rounded-full bg-muted-2/15 px-2 py-0.5 text-[11px] text-muted">No</span>;
  }
  const cls =
    variant === "warn" ? "bg-warn/15 text-warn" : "bg-danger/15 text-danger";
